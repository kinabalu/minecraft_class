...
public class BlockFillerPositionSelector extends BaseEvent {
	...
	public void choosePositions(PlayerInteractEvent event) {
		...
		if (event.action == Action.RIGHT_CLICK_BLOCK) {
			if (event.pos != null && event.pos.getX() != 0 && event.pos.getY() != 0 && event.pos.getZ() != 0) {

				pos1.clear();
				pos1.add(event.pos.getX());
				pos1.add(event.pos.getY());
				pos1.add(event.pos.getZ());
				event.entityPlayer.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN + "Position 1 set to "
			...
			if (event.pos != null && event.pos.getX() != 0 && event.pos.getY() != 0 && event.pos.getZ() != 0) {
				...
				event.entityPlayer.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN + "Position 2 set to "
						+ event.pos.getX() + ", " + event.pos.getY() + ", " + event.pos.getZ() + "."));
